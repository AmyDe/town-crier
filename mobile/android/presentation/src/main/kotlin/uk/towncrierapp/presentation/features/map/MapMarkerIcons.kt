...
private const val PIN_DIAMETER_PX = 64
private const val BUBBLE_DIAMETER_PX = 72
...
internal class MapMarkerIcons(
    ...
    fun statusIcon(status: ApplicationStatus): BitmapDescriptor =
        statusCache.getOrPut(status) { solidCircleBitmap(colorFor(status), PIN_DIAMETER_PX) }
    ...
    private fun countBubbleBitmap(label: String): BitmapDescriptor {
        val bitmap = Bitmap.createBitmap(BUBBLE_DIAMETER_PX, BUBBLE_DIAMETER_PX, Bitmap.Config.ARGB_8888)
        val canvas = Canvas(bitmap)
        val radius = BUBBLE_DIAMETER_PX / 2f
        ...
        val textPaint =
            Paint(Paint.ANTI_ALIAS_FLAG).apply {
                ...
                textSize = BUBBLE_DIAMETER_PX * BUBBLE_TEXT_SIZE_FRACTION
            }
...
/** Remembers one [MapMarkerIcons] instance per theme (colors change across light/dark/OLED), so its bitmap caches survive recomposition but regenerate on a theme switch. */
@Composable
internal fun rememberMapMarkerIcons(): MapMarkerIcons {
    ...
    return remember(statusColors, bubbleColor, bubbleTextColor) {
        MapMarkerIcons(statusColors, bubbleColor, bubbleTextColor)
    }
}

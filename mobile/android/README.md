...
1. **Create the key** in the same GCP project as FCM (#780). Maps SDK for
   Android map loads are billable Maps Platform events — set a Cloud Billing
   budget and alert on the project so unexpected usage surfaces before it
   becomes a bill, and lean on the daily quota cap below as the hard backstop.

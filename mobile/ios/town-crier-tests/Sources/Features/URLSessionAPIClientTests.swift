...
struct URLSessionAPIClientTests {
  ...
  // MARK: - 400/409 watch-zone error mapping (tc-9oyhw, GH#1085)
  ...
  func boundaryTooLargeResponse() async throws {
    ...
    transport.responses = [
      (Data(body.utf8), httpResponse(url: baseURL, statusCode: 400))
    ]
  ...
  func zoneNameTakenResponse() async throws {
    ...
    let body = #"{"error":"zone_name_taken","message":"You already have a watch zone with this name."}"#
    transport.responses = [
      (Data(body.utf8), httpResponse(url: baseURL, statusCode: 409))
    ]

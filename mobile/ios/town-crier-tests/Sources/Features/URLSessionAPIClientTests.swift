...
struct URLSessionAPIClientTests {
  ...
  @Test("On 401, when refresh succeeds but retry returns server error, propagates server error not sessionExpired")
  ...
  func retryAfterRefreshNetworkErrorThrowsNetworkUnavailable() async throws {
    ...
    // First call returns 401, second call (retry) throws URLError
    transport.responses = [
      (Data(), httpResponse(url: baseURL, statusCode: 401))
    ]
    // After the 401 response, the retry will hit the empty responses array
    // and throw URLError(.badServerResponse) from StubHTTPTransport

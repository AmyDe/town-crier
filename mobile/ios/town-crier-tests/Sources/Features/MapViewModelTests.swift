...
struct MapViewModelTests {
  ...
  // MARK: - Map Mounting (tc-edyvn)

  /// tc-edyvn: the headline regression. A zero-cluster viewport (or a zone
  /// with genuinely zero applications) used to unmount the map behind a
  /// full-screen `EmptyStateView`, trapping the user with no way to zoom back
  /// out. `showMap` is what `MapView.mapBody` now branches on instead of
  /// `isEmpty`, so it must stay true here.

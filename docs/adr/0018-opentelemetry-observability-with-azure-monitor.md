...
### 2026-06-15
- Decommissioned: **the .NET API/worker in-process instrumentation described above no longer exists.** The backend was migrated to Go and the .NET source deleted (see [ADR 0028](0028-migrate-backend-from-dotnet-to-go.md)). The manually-wired `Azure.Monitor.OpenTelemetry.Exporter`, the `TownCrier.Polling`/`TownCrier.Cosmos`/`TownCrier.PlanIt` ActivitySources and Meters, and the `SuccessfulCosmosDependencyFilter` all went with it.
- Retained at the **decision** level: OpenTelemetry remains the instrumentation standard and Azure Monitor / Application Insights remains the backend. The Go backend exports OTLP to the ACA managed-environment OTel agent, which forwards traces, dependencies (Cosmos), and logs to the same shared `appi-town-crier-shared` — see [ADR 0027](0027-go-api-observability-via-aca-otel-agent.md). The Native-AOT motivation for *manual* SDK wiring (the rejected distro package) is moot now that the backend is Go; OTLP→agent is the idiomatic Go path. The shared Application Insights resource, Log Analytics workspace, and operational dashboard are unchanged. Frontend instrumentation remains unshipped (see the 2026-04-21 amendment).

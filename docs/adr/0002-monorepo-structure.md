...
### 2026-06-18
- Replaced: the **`/infra` .NET layout (`src/town-crier.infra/` with `SharedStack.cs` / `EnvironmentStack.cs`, and `tests/town-crier.infra.tests/`) was removed.** The Pulumi program was ported to Go (see [ADR 0029](0029-migrate-infrastructure-from-dotnet-to-go.md)) with a flat layout: `infra/main.go` dispatches to `infra/shared.go` (shared stack) and `infra/environment.go` (dev/prod). `infra/global.json` is gone, replaced by `infra/go.mod` (`go 1.26`). The infra unit-test project was not carried over — validation now rests on the zero-diff `pulumi preview` CI gate.
- Unchanged: **`/cli` (.NET)** remains, and is now the only .NET component. The flat root-level structure, source/test separation principle, and the iOS Clean Architecture layout all stand.

...
1. **Red** — Write a failing test. Run `cd api && dotnet test`. Confirm failure. Commit: `"red: <test name> (<bead-id>)"`
2. **Green** — Minimum code to pass. Run `cd api && dotnet test`. Confirm pass. Commit: `"green: <test name> (<bead-id>)"`
...
- **Commit trailer** — end every commit subject with `(<bead-id>)` (e.g. `green: validates token (tc-a1b2)`). Enables `bd doctor` orphan detection.

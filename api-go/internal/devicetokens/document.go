...
// resets _ts. The 180-day TTL satisfies UK GDPR Art. 5(1)(e) storage
// limitation for device identifiers.
const deviceTTLSeconds = 180 * 24 * 60 * 60

// deviceDocument is the Cosmos persistence shape for a DeviceRegistration. The
// JSON tags use camelCase keys so documents are byte-compatible with the
// existing DeviceRegistrations container and old documents hydrate here
// unchanged.
//
...
// document id equals the token; the partition key is the user id, supplied
// separately to the store's upsert.
...
// parsing the string platform (an unknown value is an error).
